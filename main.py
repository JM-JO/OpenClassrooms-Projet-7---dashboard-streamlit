...
def rectangle_gauge(id, client_probability):
	...
	ax.add_patch(FancyArrowPatch((client_probability*100, 0.75), (client_probability*100, 0), mutation_scale=20))
	ax.set_xlim(0,100)
	ax.set_ylim(0,1)
...
def kdeplot(feature):
	...
	if str(x_client) == "nan":
		...
		plt.annotate(text=f" Client {id_client}\n  data not available", xy=(x_center,0), xytext=(x_center,y_max*0.8))
	else:
...
def contourplot(feature1, feature2):
	...
	if str(x_client) == "nan" or str(y_client) == "nan":
		...
		plt.text(s=f" Client {id_client}\n  data not available", x=x_center, y=y_center)
	else:
...
X_split_valid = load_data('X_split_valid')		# dataset used to plot features - sample of clients from the Kaggle train set
y_split_valid = load_data('y_split_valid')		# dataset used to plot features - sample of clients from the Kaggle train set
df_test_sample = load_data('df_test_sample')	# sample of customers from the Kaggle test set
