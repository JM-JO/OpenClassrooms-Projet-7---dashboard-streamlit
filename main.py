...
# Basic and Advanced Dashboards #######################################################
    ...
    if dashboard_choice == 'Advanced Dashboard':
        ...
        figure_kde_distribution_proba_default_for_datascientist = joblib.load(
            './resources/figure_kde_distribution_proba_default_for_datascientist.joblib'
        )
        plt.annotate(text=f"Client {id_client}",
                     xy=(probability, 0),
                     xytext=(probability, 3),
                     arrowprops=dict(facecolor='k', arrowstyle='simple'))
        st.pyplot(figure_kde_distribution_proba_default_for_datascientist)

        figure_hist_distribution_proba_default_for_datascientist = joblib.load(
            './resources/figure_hist_distribution_proba_default_for_datascientist.joblib'
        )
        plt.annotate(text=f"Client {id_client}",
                     xy=(probability, 0),
                     xytext=(probability, 1000),
                     arrowprops=dict(facecolor='k', arrowstyle='simple'))
    ...
    figure_features_permutation_importances_for_datascientist = joblib.load(
        './resources/figure_features_permutation_importances_for_datascientist.joblib'
    )
    st.pyplot(figure_features_permutation_importances_for_datascientist)
    st.caption(
        "For each feature, it is the average change of the predicted score after randomisation of the feature values in the dataset."
    )
    with st.expander("Features description", expanded=False):
    ...
    if dashboard_choice == 'Advanced Dashboard':
        ...
        st.pyplot(
            joblib.load(
                './resources/figure_summary_plot_shap_for_datascientist.joblib'
            ))
        with st.expander("Legend", expanded=False):
            st.caption(
                'For each feature, the higher the SHAP value, the higher is the contribution to an increase of the calculated probability of default. '
